...
const handleSave = async () => {
  ...
  try {
    await updateUserProfileApi({
      ...
      dormitoryBuilding: user.value.dormitoryBuilding,
      roomNo: user.value.roomNo,
      ...
      <UiCard variant="white" padding="24px 40px" class="info-card" elevated>
        <div class="user-header">
          <div class="user-avatar" :class="{ 'is-editable': isEditing }" @click="triggerAvatarUpload">
            ...
            <span v-else>{{ user.realName.charAt(0) }}</span>
        ...
        <div class="info-list">
          ...
          <div class="info-item">
            ...
            <div class="info-content"><span class="info-value">{{ user.studentNo }}</span></div>
          </div>
          <div class="info-item">
            <span class="info-label">宿舍楼栋</span>
            <div class="info-content">
              <span v-if="!isEditing" class="info-value">{{ user.dormitoryBuilding || '未设置' }}</span>
              <UiInput v-else v-model="user.dormitoryBuilding" placeholder="如：12号楼" />
            </div>
          </div>
          <div class="info-item">
            <span class="info-label">宿舍房号</span>
            <div class="info-content">
              <span v-if="!isEditing" class="info-value">{{ user.roomNo || '未设置' }}</span>
              <UiInput v-else v-model="user.roomNo" placeholder="如：302" />
            </div>
          </div>
          <div class="info-item">
            <span class="info-label">联系电话</span>
            <div class="info-content">
              <span v-if="!isEditing" class="info-value">{{ user.phone }}</span>
              <UiInput v-else v-model="user.phone" placeholder="输入新电话" />
            </div>
          </div>
        </div>
